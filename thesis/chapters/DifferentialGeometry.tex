...
\begin{mydef}[Heat Kernel Signature]
	...
	\begin{equation}
		HKS(x): \real^+ \rightarrow \real, HKS(x,t) = k_t(x,x) = \sum_0^{\inf} e^{-\lambda_i t}(\phi_i(x))^2.
		\label{eq:hks:def}
	\end{equation}
\end{mydef}
...
One easy counterexample is  a sphere, where $k_t(x,x)$ is the same for all $x$, but it is clearly possible to construct non-isometric maps between two spheres of the same radius.
Although this example is degenerate, we will look whether or not this has an influence on shapes with intrinsic symmetries.
...
\begin{equation}
	\begin{split}
		...
		& = \sum_i e^{-\lambda_it} (\phi_i(x) - \phi_i(y))^2.
	\end{split}
