...
A metric space being compact implies that it is closed/complete and totally bounded, which is a reasonable call, since the surfaces we are interessted in can mostly be approximated by 3D-meshes.
...
\begin{mydef}[correspondence]
	...
	\end{itemize}
\end{mydef}
Note that a correspondance neither has to map one point to exactly one other nor does every point have to be a part of the correspondence.
%TODO explain the link between map and correspondance
...
\begin{mydef}[metric distortion]
	Let $(X,d_X)$ and $(Y,d_Y)$ be compact metric spaces and $R \subset (X,d_X) \times (Y,d_Y)$ be a correspondance.
...
\begin{mydef}[Gromov-Hausdorff distance (2)]
	The Gromov-Hausdorff distance between two metric spaces $X$ and $Y$ is defined by
	$$d_\mathcal{GH}(X,Y) = \frac{1}{2} \inf_{R \subset X\timesY} dis \, R.$$
\end{mydef}
In this formulation, the Gromov-Hausdorff distance is defined as the infimum of $r > 0$ for which there is a correspondance with $dis\,R = 2r$.
And unlike the first definition, we now minimize over a finite set of correspondances instead of all possible ambient spaces and their corresponding embeddings.
% coverings, fps
